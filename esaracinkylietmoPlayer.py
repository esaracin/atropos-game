...
def get_successors(board, prev_move):
    ...
    #create successor boards from open positions
    succ = []
...
def next_move(board, depth, prev_move, turn, alpha, beta):
    """
    ...
    #all successor boards associated with their respective move created from all valid open positions
    successors = get_successors(board, prev_move)

    #for alpha-beta pruning: initializing variable that will keep track of either the max or min score so far
    if turn:
        curr = float("-inf")
    else:
        curr = float("inf")
    ...
    for succ_board, move in successors:
        ...
        if is_loss(get_neighbors(succ_board, move), move[0]):
            ...
            score, _ = next_move(succ_board, depth -1, move, not turn, alpha, beta)

            #update the maximum or minimum seen so far depending on the turn
            if turn and score > curr:
                curr = score
            elif not turn and score < curr:
                curr = score


            #if we don't need to make any more recursive calls here
            if (turn and curr >= beta) or (not turn and curr <= alpha):
                return curr, move
            else:
                #update alpha or beta depending on the turn
                if turn:
...
def main():
    # default lookahead of this player
    lookahead = 5 
    
    board, prev_move = process_board(sys.argv[1])
    _, move = next_move(board, lookahead, prev_move, True, float("-inf"), float("inf"))
